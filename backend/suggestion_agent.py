...
import json
import logging
from .config import OPENAI_API_KEY, SUGGESTION_MODEL, SUGGESTION_TEMPERATURE, SUGGESTION_MAX
from .prompt_loader import get_suggestion_system_prompt, get_suggestion_user_prompt, get_fallback_suggestions
        ...
        try:
            ...
            for idx, suggestion in enumerate(parsed[:max_suggestions]):
                if not isinstance(suggestion, dict):
                    logger.warning(f"Skipping invalid suggestion at index {idx}: not a dict")
                    continue
                
                # Ensure required fields exist
                validated = {
                    "type": suggestion.get("type", "General Suggestion"),
                    "topic": suggestion.get("topic", suggestion.get("text", "Follow up with the caller to gather more information.")),
                ...
                validated["details"].setdefault("possibleConversation", "Could you provide more details about your situation?")
                validated["details"].setdefault("priority", suggestion.get("priority", "medium"))
                ...
                if "possibleConversation" not in validated["details"]:
                    ...
                    if "operatorResponse" in validated["details"]:
                        validated["details"]["possibleConversation"] = validated["details"]["operatorResponse"]
                    elif "naturalResponse" in validated["details"]:
                        validated["details"]["possibleConversation"] = validated["details"]["naturalResponse"]
                        ...
                        if "Operator:" in conv:
                            validated["details"]["possibleConversation"] = conv.split("Operator:")[1].split("\n")[0].strip()
                        else:
